...
import { useStockStats, useStockHistory, useStockPerformance } from '../hooks/useMarket'
...
export default function StockPage() {
  ...
  const statsQ = useStockStats(symbol ?? null, exchange)
  const historyQ = useStockHistory(symbol ?? null, period, exchange)
  const perfQ = useStockPerformance(symbol ?? null, exchange)

  if (!symbol) {
    return (
      <div className="error-state">No symbol specified.</div>
    )
  }
  ...
  const dayChangePct = stats
    ? stats.last_price !== null && stats.day_high !== null
      ? null // we don't have day_change_pct directly in stats; compute from stats if available
      : null
    : null
  void dayChangePct // suppress unused warning
